...
MyHttpFile::MyHttpFile( )
{
	...
	_dlSize			= 0;		//¤w¤U¸üSize
	_isRunThread	= false;
	_state			=	 EM_MyHttpFileProcessType_None;		//¥Ø«e³B²zª¬ºA
	m_dlKbs=0;
	_port			= 0;
	_port_proxy		= 0;
	m_Got_Nothing=0;
...
void	MyHttpFile::SetUserPwd(std::string	UserPwd )
{
	_userpwd = UserPwd;
}
...
//////////////////////////////////////////////////////////////////////////
//¤U¸ü
bool	MyHttpFile::Download(std::string destFolder , std::string remoteFile , std::string localFile )
{
	if( _isRunThread != false )
		return false;
...
bool	MyHttpFile::Download_NoThread(std::string destFolder , std::string remoteFile , std::string localFile , int TryTimes )
{
...
bool	MyHttpFile::Download_NoThread(std::string destFolder , std::string remoteFile , std::string localFile )
{
	...
	while(1)
	{
		...
		switch( State() )
		{
		case EM_MyHttpFileProcessType_Download:		//¤U¸ü¤¤
			break;
		case EM_MyHttpFileProcessType_OK:			//¤U¸ü¦¨¥
			{
				return true;
			}
			break;
		case EM_MyHttpFileProcessType_None:
		case EM_MyHttpFileProcessType_Failed:		//¤U¸ü¥¢±Ñ
		case EM_MyHttpFileProcessType_Interrupt:	//­n¨D¤¤Â_
			{
				return false;
			}
			break;
...
int MyHttpFile::CurlProgressCallback( void *clientp , double dltotal , double dlnow , double ultotal , double ulnow )
{
	...
	This->_totalSize = dltotal;		//ÀÉ®×¤j¤p
	This->_dlSize	 = dlnow;		//¤w¤U¸üSize
...
#pragma endregion	//Åª¨ú¤@¯ë¸ê®Æ
//////////////////////////////////////////////////////////////////////////
#pragma region dsadsadas
