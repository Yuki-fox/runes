...
{
	int patch=(m_width+3)&~3;



		
	WORD *pNew = new WORD[patch * m_height];
	WORD *pDest = pNew;
	BYTE *pSrc = m_pPixels;
	...
	for ( int yy = 0; yy < m_height; yy++ ) 
	{
		for ( int xx = 0; xx < m_width; xx++ ) 
		{
			...
			{
				c.rgbBlue		= pSrc[(xx*3)+0];					
				c.rgbGreen		= pSrc[(xx*3)+1];
				c.rgbRed		= pSrc[(xx*3)+2];
				//c.rgbReserved	= 0xFF;
				//memcpy( &c, (BGRCOLOR*)pSrc + xx, 3 );
			} else {
				c = *((RGBQUAD*)pSrc + xx);
			}
			int r= ((c.rgbRed >>3)& 0xFF);
			int g= ((c.rgbGreen >>2)& 0xFF);
			int b= ((c.rgbBlue >>3)& 0xFF);
			pDest[xx] = (WORD)(( r<< 11) | (g << 5 )| b);
		}
		pDest += m_width;
...
void cImage::ConvertToA1R5G5B5()
{
	...
	for ( int yy = 0; yy < m_height; yy++ ) {
		for ( int xx = 0; xx < m_width; xx++ ) {
			...
			int r= ((c.rgbRed >>3)& 0xFF);
			int g= ((c.rgbGreen >>3)& 0xFF);
			int b= ((c.rgbBlue >>3)& 0xFF);
			//int a=c.rgbReserved>>7;

			if ( c.rgbReserved ) 
			{
				pDest[xx] = 0x8000 | r << 10 | g << 5 | b >> 3;
			} else {
				pDest[xx] = r << 10 | g << 5 | b >> 3;
			}
...
bool cImage::Decode( SQStream* file, ImageLibFormat format )
{
	if ( file == NULL || m_pPixels != NULL || m_pPalette != NULL )
	{
		return false;
	}
	...
	case IMAGELIB_FORMAT_UNKNOWN:
		{
#if IMAGELIB_SUPPORT_BMP
			if ( Decode( file, IMAGELIB_FORMAT_BMP ) ) return true;
#endif
#if IMAGELIB_SUPPORT_GIF
			if ( Decode( file, IMAGELIB_FORMAT_GIF ) ) return true;
#endif
#if IMAGELIB_SUPPORT_JPG
			if ( Decode( file, IMAGELIB_FORMAT_JPG ) ) return true;
#endif
#if IMAGELIB_SUPPORT_PCX
			if ( Decode( file, IMAGELIB_FORMAT_PCX ) ) return true;
#endif
#if IMAGELIB_SUPPORT_PNG
			if ( Decode( file, IMAGELIB_FORMAT_PNG ) ) return true;
#endif
#if IMAGELIB_SUPPORT_TGA
			if ( Decode( file, IMAGELIB_FORMAT_TGA ) ) return true;
#endif
			break;
		}
#if IMAGELIB_SUPPORT_BMP
...
bool cImage::Load( SQStream* file, ImageLibFormat format  )
{
bool retBool;

retBool = Decode( file, format );
return retBool;
}

bool cImage::Load( const TCHAR *fileNameStr, ImageLibFormat format )
{
	FILE *fptr;
	bool retBool;
	if ( (fptr = _wfopen( fileNameStr, _T("rb") )) == NULL ) {
		return false;
	}
	retBool = Decode( fptr, format );
	fclose( fptr );	

	return retBool;
}

bool cImage::Decode( FILE *fptr, ImageLibFormat format )
{
	if ( fptr == NULL || m_pPixels != NULL || m_pPalette != NULL ) {
		return false;
	}
	...
	case IMAGELIB_FORMAT_UNKNOWN:
		{
#if IMAGELIB_SUPPORT_BMP
			if ( Decode( fptr, IMAGELIB_FORMAT_BMP ) ) return true;
#endif
#if IMAGELIB_SUPPORT_GIF
			if ( Decode( fptr, IMAGELIB_FORMAT_GIF ) ) return true;
#endif
#if IMAGELIB_SUPPORT_JPG
			if ( Decode( fptr, IMAGELIB_FORMAT_JPG ) ) return true;
#endif
#if IMAGELIB_SUPPORT_PCX
			if ( Decode( fptr, IMAGELIB_FORMAT_PCX ) ) return true;
#endif
#if IMAGELIB_SUPPORT_PNG
			if ( Decode( fptr, IMAGELIB_FORMAT_PNG ) ) return true;
#endif
#if IMAGELIB_SUPPORT_TGA
			if ( Decode( fptr, IMAGELIB_FORMAT_TGA ) ) return true;
#endif
			break;
		}
#if IMAGELIB_SUPPORT_BMP
...
bool cImage::Save( const TCHAR *fileNameStr, ImageLibFormat format )
{
	FILE *fptr;
	bool retBool;
	if ( (fptr = _wfopen( fileNameStr, _T("wb") )) == NULL ) {
		return false;
	}
	retBool = Encode( fptr, format );
	fclose( fptr );	

	return retBool;
}

bool cImage::Encode( FILE *fptr, ImageLibFormat format )
{
	if ( fptr == NULL || m_pPixels == NULL ) {
		return false;
	}
