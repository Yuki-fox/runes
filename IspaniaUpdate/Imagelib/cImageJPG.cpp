...
void JPEG_init_sourceX(j_decompress_ptr cinfo);
boolean JPEG_fill_input_bufferX(j_decompress_ptr cinfo);
void JPEG_skip_input_dataX(j_decompress_ptr cinfo, long num_bytes);
void JPEG_term_sourceX(j_decompress_ptr cinfo);
void JPEG_error_exitX(j_common_ptr cinfo);
void JPEG_emit_messageX(j_common_ptr cinfo, int msg_level);

bool cImageJPG::Decode( SQStream*file )
{
	if ( file == NULL )	
		return false;
	...
	mgr.bytes_in_buffer = 0;
	mgr.next_input_byte = NULL;
	mgr.init_source       = JPEG_init_sourceX;
	mgr.fill_input_buffer = JPEG_fill_input_bufferX;
	mgr.skip_input_data   = JPEG_skip_input_dataX;
	mgr.resync_to_restart = jpeg_resync_to_restart;  // use default
	mgr.term_source       = JPEG_term_sourceX;
...
boolean JPEG_fill_input_bufferX(j_decompress_ptr cinfo)
{
	JPGInternalStructX* is = (JPGInternalStructX*)(cinfo->client_data);

	


	cinfo->src->bytes_in_buffer = is->io->Read(is->buffer,JPEG_BUFFER_SIZE);
	cinfo->src->next_input_byte = is->buffer;
	return (cinfo->src->bytes_in_buffer != 0);
}
