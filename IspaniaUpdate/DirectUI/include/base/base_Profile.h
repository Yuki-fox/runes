#ifndef MING_BASE_PROFILE_H
#define MING_BASE_PROFILE_H
...
public:
	 ...
	 */
	Profile( int id, const SQChar* sz )	: m_id(id) {if (!sm_names[id][0]) setName(id,sz); m_time=rdtsc();}
	 ...
	 */
	static const SQChar*	getName( int i );
...
private:
	...
	static SQChar		sm_names[MAX_BLOCKS][MAX_NAME];

	static void		init();
	static int		indexToId( int i );
	static void		setName( int id, const SQChar* sz );
};
 ...
 */
#define PROFILE(BLOCKNAME) static const int s_id_ ## BLOCKNAME = Profile::id(); Profile pr_ ## BLOCKNAME( s_id_ ## BLOCKNAME, _SC(#BLOCKNAME ));


} // base


#endif // MING_BASE_PROFILE_H
