...
SQChar	Profile::sm_names[MAX_BLOCKS][MAX_NAME];


void Profile::setName( int id, const SQChar* sz )
{
	...
	scstrncpy( sm_names[id], sz, MAX_NAME-1 );
...
const SQChar*	Profile::getName( int i )
{
...
void Profile::init()
{
	...
	{
		throwError( Exception(Format(_SC("High resolution timer not available. Re-compile application with PROFILE_ENABLED #undefined."))) );
	}
}
