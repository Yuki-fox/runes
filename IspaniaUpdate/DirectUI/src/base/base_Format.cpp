...
int Format::format( SQChar* buffer, int size ) const
{
	int		d		= 0;
	int		s		= 0;
	bool	skip	= false;
	...
	while ( d < size && s < (int)m_pattern.length() )
	{
		...
		if ( ch == '{' && !skip && s < m_pattern.length() )
		{
			...
			if ( -1 == end )
				throwError( FormatException( Format(_SC("Failed to format \"{0}\", '}' not found after {1}"), m_pattern, s) ) );
			if ( end-s == 0 )
				throwError( FormatException( Format(_SC("Failed to format \"{0}\", argument index not found at {1}"), m_pattern, s) ) );

			// argument index
			SQChar digit = m_pattern.charAt( s++ );
			if ( !Character::isDigit(digit) )
				throwError( FormatException( Format(_SC("Failed to format \"{0}\", digit expected at {1}"), m_pattern, s-1) ) );
			int arg = digit - '0';
			if ( !(arg >= 0 && arg < m_args) )
				throwError( FormatException( Format(_SC("Failed to format \"{0}\", invalid argument index at {1}"), m_pattern, s-1) ) );
