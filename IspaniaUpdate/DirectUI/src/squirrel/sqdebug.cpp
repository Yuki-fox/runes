...
void SQVM::Raise_Error(const SQChar *s, ...)
{
	...
	va_end(vl);
	_lasterror = SQString::Create(_spval,-1);
}
...
SQString *SQVM::PrintObjVal(const SQObject &o)
{
	...
	case OT_FLOAT:
		...
		break;
	default:
		return SQString::Create( GetTypeName(o));
	}
}
...
{
	SQObjectPtr exptypes = SQString::Create( _SC(""), -1);
	SQInteger found = 0;	
	for(SQInteger i=0; i<16; i++)
	{
		SQInteger mask = 0x00000001 << i;
		if(typemask & (mask)) {
			if(found>0) StringCat(exptypes,SQString::Create( _SC("|"), -1), exptypes);
			found ++;
			StringCat(exptypes,SQString::Create( IdType2Name((SQObjectType)mask), -1), exptypes);
		}
	}
