...
{
	SQObjectPtr &o = stack_get(v,2);
	v->Push(SQString::Create(GetTypeName(o),-1));
	return 1;
}
...
static SQInteger closure_getinfos(HSQUIRRELVM v) {
	...
	if(type(o) == OT_CLOSURE) {
		...
		if(f->_varparams) {
			_array(params)->Set(nparams-1,SQString::Create(_SC("..."),-1));
		}
		res->NewSlot(SQString::Create(_SC("native"),-1),false);
		res->NewSlot(SQString::Create(_SC("name"),-1),f->_name);
		res->NewSlot(SQString::Create(_SC("src"),-1),f->_sourcename);
		res->NewSlot(SQString::Create(_SC("parameters"),-1),params);
		res->NewSlot(SQString::Create(_SC("varargs"),-1),f->_varparams);
	}
	else { //OT_NATIVECLOSURE 
		SQNativeClosure *nc = _nativeclosure(o);
		res->NewSlot(SQString::Create(_SC("native"),-1),true);
		res->NewSlot(SQString::Create(_SC("name"),-1),nc->_name);
		res->NewSlot(SQString::Create(_SC("paramscheck"),-1),nc->_nparamscheck);
		...
		res->NewSlot(SQString::Create(_SC("typecheck"),-1),typecheck);
	}
	v->Push(res);
	return 1;
}
...
static SQInteger generator_getstatus(HSQUIRRELVM v)
{
	...
	switch(_generator(o)->_state){
		case SQGenerator::eSuspended:v->Push(SQString::Create(_SC("suspended")));break;
		case SQGenerator::eRunning:v->Push(SQString::Create(_SC("running")));break;
		case SQGenerator::eDead:v->Push(SQString::Create(_SC("dead")));break;
	}
	return 1;
}
