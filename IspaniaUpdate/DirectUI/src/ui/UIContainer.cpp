...
CContainerUI::CContainerUI(SquirrelObject&so) : CControlUI(so),
   m_hwndScroll(NULL), 
   m_iPadding(0),
   m_iScrollPos(0),
   m_bAutoDestroy(true),
...
{
	base::Debug::printf(_T("~CContainerUI %s"),GetClass());
   RemoveAll();
}
...
{
   for( int it = 0; m_bAutoDestroy && it < m_items.size(); it++ ) 
   {
...
{
	//CBlueRenderEngineUI::DoPaintFrame(hDC, m_pManager, m_rcItem,UICOLOR_STANDARD_RED ,UICOLOR_STANDARD_GREY);
   RECT rcTemp = { 0 };
...
void CContainerUI::ProcessScrollbar(RECT rc, int cyRequired)
{
   ...
   if( cyRequired > rc.bottom - rc.top && m_hwndScroll == NULL && m_bAllowScrollbars ) 
   {
      ...
      ::SetProp(m_hwndScroll, _SC("WndX"), static_cast<HANDLE>(this));
...
CCanvasUI::CCanvasUI(SquirrelObject&so) : CContainerUI(so),
m_hBitmap(NULL), m_iOrientation(HTBOTTOMRIGHT)
{
}
...
bool CCanvasUI::SetWatermark(LPCTSTR pstrBitmap, int iOrientation)
{
   ...
   m_hBitmap = DirectUI::LoadImage(pstrBitmap,255,255,255);

	   //(HBITMAP) ::LoadImage(m_pManager->GetResourceInstance(), pstrBitmap, IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION| LR_LOADFROMFILE);
   ...
   if( m_hBitmap != NULL )
   {
	 ...
	 if( m_hBitmap != NULL )
	 {
	  // DWORD dwTimeDiff = ::timeGetTime() - m_dwFadeTick;
	   TPostPaintUI job;
	   job.rc = rcBitmap;
	   job.hBitmap = m_hBitmap;
	   job.iAlpha = 64;//(BYTE) CLAMP(255 - (long)(dwTimeDiff / (FADE_DELAY / 255.0)), 0, 255);
...
CWindowCanvasUI::CWindowCanvasUI(SquirrelObject&so) : CCanvasUI(so)
{
...
CControlCanvasUI::CControlCanvasUI(SquirrelObject&so) : CCanvasUI(so)
{
   SetInset(CSize(0, 0));
...
CWhiteCanvasUI::CWhiteCanvasUI(SquirrelObject&so) : CCanvasUI(so)
{
   SetInset(CSize(0, 0));
...
CDialogCanvasUI::CDialogCanvasUI(SquirrelObject&so) : CCanvasUI(so)
{
   SetInset(CSize(10, 10));
   m_clrBack = m_pManager->GetThemeColor(UICOLOR_WINDOW_BACKGROUND);
...
CTabFolderCanvasUI::CTabFolderCanvasUI(SquirrelObject&so) : CCanvasUI(so)
{
   SetInset(CSize(0, 0));
   COLORREF clrColor1;
...
CVerticalLayoutUI::CVerticalLayoutUI(SquirrelObject&so) : CContainerUI(so), m_cyNeeded(0)
{
}
...
CHorizontalLayoutUI::CHorizontalLayoutUI(SquirrelObject&so) : CContainerUI(so)
{
}
...
CTileLayoutUI::CTileLayoutUI(SquirrelObject&so) : CContainerUI(so), m_nColumns(2), m_cyNeeded(0)
{
   SetPadding(10);
   SetInset(CSize(10, 10));
}
...
void CTileLayoutUI::SetPos(RECT rc)
{
   ...
   rc.right -= m_rcInset.right;
   rc.bottom -= m_rcInset.bottom;
   int width=m_pManager?m_pManager->GetSystemMetrics().cxvscroll:12;
   if( m_hwndScroll != NULL ) rc.right -= width;
   // Position the elements
...
CDialogLayoutUI::CDialogLayoutUI(SquirrelObject&so) : CContainerUI(so),
m_bFirstResize(true), m_aModes(sizeof(STRETCHMODE))
{
...
void CDialogLayoutUI::RecalcArea()
{
   ...
   {         
      CControlUI* pControl = static_cast<CControlUI*>(m_items[it]);
      bool bFound = false;
      for( int i = 0; !bFound && i < m_aModes.size(); i++ )
	  {
      ...
      if( !bFound ) 
	  {
         ...
         mode.rcItem = pControl->GetEstimatePos();
         m_aModes.Add(&mode);
      }
   }
