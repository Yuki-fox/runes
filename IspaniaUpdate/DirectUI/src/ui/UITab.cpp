...
CTabFolderUI::CTabFolderUI(SquirrelObject&so) :CContainerUI(so),
m_iCurSel(-1), m_pCurPage(NULL), m_aTabAreas(sizeof(RECT))
{
...
void CTabFolderUI::Event(TEventUI& event)
{
   ...
   {
      for( int i = 0; i < m_items.size() && i < m_aTabAreas.size(); i++ ) 
	  {
...
{
   {
      ...
      if( ::IntersectRect(&rcTemp, &rcPaint, &rcTabs) ) 
      {
         ...
         for( int i = 0; i < GetCount(); i++ ) 
         {
            ...
            const base::String& strText = pPage->GetText();
...
CTabPageUI::CTabPageUI(SquirrelObject&so):CContainerUI(so)
{
   SetInset(CSize(6, 6));
}
