...
CLabelPanelUI::CLabelPanelUI(SquirrelObject&so) :CControlUI(so),
m_cxWidth(0), m_uTextStyle(DT_VCENTER)
{
}
...
void CLabelPanelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
   ...
   else if ( _tcscmp(pstrName, _T("multi")) == 0 )
   {
...
CGreyTextHeaderUI::CGreyTextHeaderUI(SquirrelObject&so):CControlUI(so)
{
