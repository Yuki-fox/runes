...
HIMAGELIST m_himgIconsCheckBox = NULL;
/////////////////////////////////////////////////////////////////////////////////////
   ...
   m_aPostPaint(sizeof(TPostPaintUI))
{
   if( m_hFonts[1] == NULL ) 
   {
      ...
      m_aLogFonts[UIFONT_LINK] = lfLink;
	   ...
	   lfExt.lfWeight += FW_BOLD;
	   _tcscpy(lfExt.lfFaceName, _T("Verdana"));
	   m_aLogFonts[UIFONT_EXT] = lfExt;

	   LOGFONT lfExt2 = lf;
	   lfExt2.lfHeight = -11;
	   _tcscpy(lfExt2.lfFaceName, _T("Verdana"));
	   m_aLogFonts[UIFONT_EXT2] = lfExt2;
...
void CPaintManagerUI::Finish(void)
{


	m_aPreMessages.Empty();
}
...
LPCTSTR CPaintManagerUI::LoadFromResource(int ID)
{
	HINSTANCE   ins=GetLanguageInstance();  
	HRSRC hRes=FindResource(ins,MAKEINTRESOURCE(ID) ,_T("NUT"));//IDR_GBTOBIG   
	HGLOBAL hGlobal=LoadResource(ins,hRes);   //!!!!!  

	DWORD dwSize = SizeofResource(ins, hRes); 
	...
	char * hMem=new char [dwSize+2];
	memset(hMem,0,dwSize+2);

	//HGLOBAL hMem = ::GlobalAlloc(GMEM_MOVEABLE, dwSize); 

	memcpy(hMem, pNut, dwSize); 
...
bool CPaintManagerUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& lRes)
{
   ...
   for( int i = 0; i < m_aMessageFilters.size(); i++ ) 
   {
      bool bHandled = false;
   ...
   case WM_APP + 1:
      {
         ...
         for( int i = 0; i < m_aDelayedCleanup.size(); i++ ) 
			 delete static_cast<CControlUI*>(m_aDelayedCleanup[i]);
   ...
   case WM_NCHITTEST:
	   {
		   ...
		   leftTop.x=rcClient.left;
		   leftTop.y=rcClient.top;
		   rightBottom.x=rcClient.right;
		   rightBottom.y=rcClient.bottom;

		   
		   ::ClientToScreen(m_hWndPaint,&leftTop);
		   ::ClientToScreen(m_hWndPaint,&rightBottom);
		 


			if (pt.x>= leftTop.x &&pt.y>=leftTop.y &&pt.x<rightBottom.x &&pt.y<rightBottom.y)
		   lRes=   HTCAPTION ;
			else
				 lRes= 0;
		   }
         ...
         else*/
         {
            ...
            if( m_bOffscreenPaint )
            {
			   ...
			   if (m_hbmpBackGround)
			   {
					...
					HDC memDC;
					memDC=::CreateCompatibleDC(m_hDcOffscreen);
				  HBITMAP  OldBitmap=(HBITMAP)::SelectObject(memDC,m_hbmpBackGround);
               ...
               for( int i = 0; i < m_aPostPaint.size(); i++ ) 
			   {
   ...
   case WM_TIMER:
      {
         for( int i = 0; i < m_aTimers.size(); i++ )
		 {
   ...
   case WM_MOUSEHOVER:
      {
         ...
         base::String sToolTip = pHover->GetToolTip();
         if( sToolTip.empty() ) 
			 return true;
         ProcessResourceTokens(sToolTip);
   ...
   case WM_MOUSELEAVE:
      {
         ...
         if( m_bMouseTracking ) 
			 ::SendMessage(m_hWndPaint, WM_MOUSEMOVE, 0, (LPARAM)lParam);
         m_bMouseTracking = false;
      }
      break;
   case WM_MOUSEMOVE:
   ...
   case WM_LBUTTONDOWN:
      {
       ...
       //  m_anim.CancelJobs();
		 ...
		 if (pControl->GetClass()==_T("SQButton"))
		   return true;
      }
      break;
   case WM_LBUTTONUP:
   ...
   case WM_VSCROLL:
      {
         if( lParam == NULL ) 
			 break;
         CContainerUI* pContainer = static_cast<CContainerUI*>(::GetProp((HWND) lParam, _SC("WndX")));
         if( pContainer == NULL ) 
			 break;
         TEventUI event = { 0 };
 ...
 void CPaintManagerUI::SetBackGround(HBITMAP bg,int Width,int Height)
 {
	 if (m_hbmpBackGround)
		 ::DeleteObject(m_hbmpBackGround);
	 ...
	 m_szWindow.cx=Width;
	 m_szWindow.cy=Height;



	 m_hbmpBackGround=bg;
 }
 void CPaintManagerUI::SetWindowRgn(HRGN hRegion)
 {

	 HWND parent=::GetParent(m_hWndPaint);

	 if (parent)
		 ::SetWindowRgn(parent,hRegion,TRUE);


	// ::SetWindowRgn(m_hWndPaint,hRegion,TRUE);
...
void CPaintManagerUI::Invalidate(RECT rcItem)
{
 ...
 void CPaintManagerUI::PostMessageX(int msg)
 {
	 ::PostMessage(m_hWndPaint, msg, 0, 0L);
 }
...
bool CPaintManagerUI::TranslateMessage(const LPMSG pMsg)
{
   ...
   LRESULT lRes = 0;
   for( int i = 0; i < m_aPreMessages.size(); i++ ) 
   {
      CPaintManagerUI* pT = static_cast<CPaintManagerUI*>(m_aPreMessages[i]);
	   HWND hwnd = pT->GetPaintWindow();
      if( pMsg->hwnd == hwnd
         || (hwndParent == hwnd && ((uStyle & WS_CHILD) != 0)) )
      {
...
{
   for( int i = 0; i < m_aNotifiers.size(); i++ ) 
   {
...
HFONT CPaintManagerUI::GetThemeFont(UITYPE_FONT Index) const
{
   if( Index <= UIFONT__FIRST || Index >= UIFONT__LAST ) 
	   return NULL;
   if( m_hFonts[Index] == NULL ) m_hFonts[Index] = 
	   ::CreateFontIndirect(&m_aLogFonts[Index]);
...
HICON CPaintManagerUI::GetThemeIcon(int iIndex, int cxySize) const
{
   if( m_himgIcons16 == NULL ) 
   {
	  ...
	  m_himgIconsCheckBox=ImageList_LoadImage(m_hInstance, MAKEINTRESOURCE(IDB_CHECKBOX), 16, 0, RGB(255,0,255), IMAGE_BITMAP, LR_CREATEDIBSECTION|LR_COLOR);
   }
   if( cxySize == 16 ) 
   ...
   return ImageList_GetIcon(m_himgIconsCheckBox,iIndex,ILD_NORMAL);
}
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromNameHash(CControlUI* pThis, LPVOID pData)
{
   ...
   // No name?
   const base::String& sName = pThis->GetName();
   if( sName.empty() ) 
	   return NULL;
   // Add this control to the hash list
   int nCount = 0;
   int nSize = pManager->m_aNameHash.size();
...
//
//
int CControlUI::s_count=0;
CControlUI::CControlUI(SquirrelObject&so) : SOCount(so),
   m_pManager(NULL), 
   m_pParent(NULL), 
   m_pTag(NULL),
   m_chShortcut('\0'),
   ...
   m_bEnabled(true)
{
	 base::Debug::printf(_T("CControlUI() %d "),s_count);
	 s_count++;
   ::ZeroMemory(&m_rcItem, sizeof(RECT));
   ::ZeroMemory(&m_rcEstimate, sizeof(RECT));
...
CControlUI::~CControlUI()
{
   ...
   base::Debug::printf(_T("%d ~CControlUI()%s"),s_count,m_sText.c_str());
}
...
base::String CControlUI::GetText() const
{
   return m_sText;
}
...
base::String CControlUI::GetToolTip() const
{
   return m_sToolTip;
}
...
base::String CControlUI::GetName() const
{
   return m_sName;
}
...
RECT CControlUI::GetEstimatePos() const
{

	return m_rcEstimate;
}
void CControlUI::SetEstimatePos(RECT rc)
{
	m_rcEstimate=rc;
}
...
CControlUI* CControlUI::ApplyAttributeList(LPCTSTR pstrList)
{
   ...
   int i,j;
   SQChar sValue[2048];
	const SQChar* pSqChar=pstrList;
   while( *pSqChar != '\0' ) 
   {
	   i=0;
	   j=0;
     
      while( *pSqChar != '\0' && *pSqChar != '=' ) 
		  sItem[i++] = *pSqChar++;
      ASSERT(*pSqChar=='=');
      if( *pSqChar++ != '=' ) 
		  return this;
