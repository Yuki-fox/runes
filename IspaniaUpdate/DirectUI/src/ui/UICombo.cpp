...
CSingleLinePickUI::CSingleLinePickUI(SquirrelObject&so) : CControlUI(so),m_cxWidth(0), m_nLinks(0), m_uButtonState(0)
{
...
class CDropDownWnd : public CWindowHost
{
public:
...
LPCTSTR CDropDownWnd::GetWindowClassName() 
{
...
LRESULT CDropDownWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   {     
	  ...
	  SquirrelObject ih=SquirrelVM::CreateInstance(rt.GetValue(_T("ControlCanvas")));
	 
	
      CControlCanvasUI* pWindow = new CControlCanvasUI(ih);

	  ih.SetInstanceUP(pWindow);
	  ih=SquirrelVM::CreateInstance(rt.GetValue(_T("VerticalLayout")));
      CVerticalLayoutUI* pLayout = new CVerticalLayoutUI(ih);
	  ih.SetInstanceUP(pLayout);
      for( int i = 0; i < m_pOwner->GetCount(); i++ ) 
		  pLayout->Add(static_cast<CControlUI*>(m_pOwner->GetItem(i)));
...
CDropDownUI::CDropDownUI(SquirrelObject&so) : CContainerUI(so),
   m_iCurSel(-1), 
   m_uButtonState(0)
{
...
base::String CDropDownUI::GetText() const
{
   if( m_iCurSel < 0 ) 
	   return _T("");
   CControlUI* pControl = static_cast<CControlUI*>(m_items[m_iCurSel]);
...
SIZE CDropDownUI::EstimateSize(SIZE /*szAvailable*/)
{
   ...
   if( m_cxyFixed.cx > 0 && m_items.size() )
   {
      RECT rcText = m_rcItem;
      ::InflateRect(&rcText, -4, -2);
