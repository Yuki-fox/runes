...
void CBlueRenderEngineUI::DoPaintBitmap(HDC hDC, CPaintManagerUI* pManager, HBITMAP hBmp, RECT DestRc,RECT rcBitmap)
{
	ASSERT(::GetObjectType(hBmp)==OBJ_BITMAP);
	ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
	// Paint image
	HDC hdcBmp = ::CreateCompatibleDC(hDC);
	HBITMAP hOldBitmap = (HBITMAP) ::SelectObject(hdcBmp, hBmp);
	::StretchBlt(hDC, DestRc.left, DestRc.top, DestRc.right - DestRc.left, DestRc.bottom - DestRc.top, hdcBmp,rcBitmap.left, rcBitmap.top,rcBitmap.right-rcBitmap.left,rcBitmap.bottom-rcBitmap.top, SRCCOPY);
	::SelectObject(hdcBmp, hOldBitmap);
...
void CBlueRenderEngineUI::DoPaintButton2(HDC hDC, CPaintManagerUI* pManager, RECT rc,HBITMAP* hBmpList ,UINT uState)
{
	...
	if (hBitmap)
	DoPaintBitmap(hDC, pManager, hBitmap, rc,bitRc);
...
void CBlueRenderEngineUI::DoPaintCheckBox(HDC hDC, CPaintManagerUI* pManager, RECT rcItem, LPCTSTR pstrText, UINT uState, UINT uStyle)
{
	ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
	// Determine placement of elements
	RECT rcText = rcItem;
	...
	bool bSelected = (uState & UISTATE_CHECKED) != 0;
	int iIcon = bSelected ? 2 : 1;
	if( (uState & UISTATE_PUSHED) != 0 ) 
		iIcon = 3;
	if( (uState & UISTATE_DISABLED) != 0 ) 
		iIcon = bSelected ? 5 : 4;
	HICON hIcon = pManager->GetThemeIcon(iIcon, 0);
	::DrawIconEx(hDC, rcButton.left, rcButton.top, hIcon, 16, 16, 0, NULL, DI_NORMAL);
	::DestroyIcon(hIcon);
	...
	if( ((uState & UISTATE_FOCUSED) != 0) && pManager->GetSystemSettings().bShowKeyboardCues )
	{
		...
		HICON hIcon = pManager->GetThemeIcon(0, 0);
		::DrawIconEx(hDC, rcButton.left, rcButton.top, hIcon, 16, 16, 0, NULL, DI_NORMAL);
		::DestroyIcon(hIcon);
...
void CBlueRenderEngineUI::DoPaintTabFolder(HDC hDC, CPaintManagerUI* pManager, RECT& rcItem, LPCTSTR pstrText, UINT uState)
{
	...
	::SetBkMode(hDC, TRANSPARENT);
	::SelectObject(hDC, pManager->GetThemeFont(UIFONT_NORMAL));
	base::String sText = pstrText;
	ProcessResourceTokens(sText);
	int cchText = sText.length();
	SIZE szText = { 0 };
...
void CBlueRenderEngineUI::DoPaintGradient(HDC hDC, CPaintManagerUI* pManager, RECT rc, COLORREF clrFirst, COLORREF clrSecond, bool bVertical, int nSteps)
{
	...
	static PGradientFill lpGradientFill = (PGradientFill) ::GetProcAddress(::GetModuleHandle(_SC("msimg32.dll")), "GradientFill");
...
void CBlueRenderEngineUI::DoPaintAlphaBitmap(HDC hDC, CPaintManagerUI* pManager, HBITMAP hBitmap, RECT rc, BYTE iAlpha)
{
	...
	static LPALPHABLEND lpAlphaBlend = (LPALPHABLEND) ::GetProcAddress(::GetModuleHandle(_SC("msimg32.dll")), "AlphaBlend");
...
{
	typedef BOOL (WINAPI *LPALPHABLEND)(HDC, int, int, int, int,HDC, int, int, int, int, BLENDFUNCTION);
	static FARPROC lpAlphaBlend = ::GetProcAddress(::GetModuleHandle(_SC("msimg32.dll")), "AlphaBlend");
	if( lpAlphaBlend == NULL )
		return NULL;
	int cx = rc.right - rc.left;
	int cy = rc.bottom - rc.top;
