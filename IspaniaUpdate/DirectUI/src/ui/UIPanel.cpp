...
CNavigatorPanelUI::CNavigatorPanelUI(SquirrelObject&so) :CVerticalLayoutUI(so), m_iCurSel(-1)
{
}
...
CNavigatorButtonUI::CNavigatorButtonUI(SquirrelObject&so) :CListElementUI(so),
m_uButtonState(0)
{
}
...
CTaskPanelUI::CTaskPanelUI(SquirrelObject&so) :CVerticalLayoutUI(so),
m_hFadeBitmap(NULL)
{
   SetInset(CSize(8, 8));
   SetPadding(10);
...
CSearchTitlePanelUI::CSearchTitlePanelUI(SquirrelObject&so) :CHorizontalLayoutUI(so),
m_iIconIndex(-1)
{
   SetInset(CSize(0, 0));
}
...
CPaddingPanelUI::CPaddingPanelUI(SquirrelObject&so):CControlUI(so)
{
...
{
	//RECT rc1 = { m_rcItem.left, m_rcItem.top , m_rcItem.right , m_rcItem.top };
	//CBlueRenderEngineUI::DoPaintRectangle(hDC, m_pManager, m_rcItem,UICOLOR_STANDARD_RED, UICOLOR_STANDARD_WHITE);
	//RECT rc2 = { m_rcItem.left-4, m_rcItem.top - 4, m_rcItem.right+4, m_rcItem.top + 4 };
	//CBlueRenderEngineUI::DoPaintLine(hDC, m_pManager, rc2, UICOLOR_EDIT_TEXT_DISABLED);
}
...
CImagePanelUI::CImagePanelUI(SquirrelObject&so) :CControlUI(so),
m_hBitmap(NULL)
{
   m_cxyFixed.cx = m_cxyFixed.cy = 0;
}
...
bool CImagePanelUI::SetImage(LPCTSTR pstrBitmap)
{
   if( m_hBitmap != NULL )
	   ::DeleteObject(m_hBitmap);
   m_hBitmap = (HBITMAP)DirectUI::LoadImage(pstrBitmap,1,1,1);
	 //  ::LoadImage(m_pManager->GetResourceInstance(), pstrBitmap, IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
   ASSERT(m_hBitmap);
   if( m_hBitmap == NULL )
	   return false;
   BITMAP bm = { 0 };
...
CTextPanelUI::CTextPanelUI(SquirrelObject&so) : CLabelPanelUI(so)
,m_nLinks(0), m_uButtonState(0), m_TextColor(UICOLOR_EDIT_TEXT_NORMAL), m_BackColor(UICOLOR__INVALID)
{
...
CWarningPanelUI::CWarningPanelUI(SquirrelObject&so) : CTextPanelUI(so),
