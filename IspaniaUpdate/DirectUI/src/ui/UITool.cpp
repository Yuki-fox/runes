...
CStatusbarUI::CStatusbarUI(SquirrelObject&so):CContainerUI(so)
{
...
{
	int height=m_pManager? m_pManager->GetThemeFontInfo(UIFONT_CAPTION).tmHeight:12;
   return CSize(0, 10 +height);
}
...
CToolbarTitlePanelUI::CToolbarTitlePanelUI(SquirrelObject&so):CControlUI(so),
m_iPadding(6)
{
}
...
CToolbarUI::CToolbarUI(SquirrelObject&so):CHorizontalLayoutUI(so)
{
   SetInset(CSize(2, 2));
   SetPadding(2);
}
...
CToolButtonUI::CToolButtonUI(SquirrelObject&so):CButtonUI(so)
{
}
...
//
//
CToolSeparatorUI::CToolSeparatorUI(SquirrelObject&so) :CControlUI(so)
{
...
CToolGripperUI::CToolGripperUI(SquirrelObject&so):CControlUI(so)
{
