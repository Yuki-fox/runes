...
class CSingleLineEditWnd : public CWindowHost
{
public:
   CSingleLineEditWnd();
...
void CSingleLineEditWnd::Init(CSingleLineEditUI* pOwner)
{
   ...
   HWND client=pOwner->GetManager()->GetPaintWindow();
   RECT rc;
   GetWindowRect(client,&rc);
   Create(client, NULL, WS_CHILD | pOwner->m_uEditStyle, 0, rcPos);
...
LPCTSTR CSingleLineEditWnd::GetWindowClassName() 
{
...
CSingleLineEditUI::CSingleLineEditUI(SquirrelObject&so) :CControlUI(so)
, m_pWindow(NULL), m_uEditStyle(ES_AUTOHSCROLL), m_bReadOnly(false)
{
}
...
class CMultiLineEditWnd : public CWindowHost
{
public:
...
LPCTSTR CMultiLineEditWnd::GetWindowClassName() 
{
...
CMultiLineEditUI::CMultiLineEditUI(SquirrelObject&so) :CControlUI(so), m_pWindow(NULL)
{
}
...
base::String CMultiLineEditUI::GetText() const
{
   if( m_pWindow != NULL ) 
   {
