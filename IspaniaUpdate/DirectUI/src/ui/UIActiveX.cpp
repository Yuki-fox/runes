...
class CActiveXWnd : public CWindowHost
{
public:
...
{
   TRACE(_SC("AX: CActiveXCtrl::SetSite"));
   if( m_pUnkSite != NULL ) 
   {
      m_pUnkSite->Release();
      m_pUnkSite = NULL;
...
{
   TRACE(_SC("AX: CActiveXCtrl::GetSite"));
   if( ppvSite == NULL ) 
	   return E_POINTER;
   *ppvSite = NULL;
   if( m_pUnkSite == NULL ) 
...
{
   TRACE(_SC("AX: CActiveXCtrl::GetContainer"));
   if( ppContainer == NULL )
	   return E_POINTER;
   *ppContainer = NULL;
...
{
   TRACE(_SC("AX: CActiveXCtrl::ShowObject"));
   if( m_pOwner == NULL ) 
	   return E_UNEXPECTED;
   HDC hDC = ::GetDC(m_pOwner->m_hwndHost);
   if( hDC == NULL ) return E_FAIL;
   if( m_pViewObject != NULL ) 
	   m_pViewObject->Draw(DVASPECT_CONTENT, -1, NULL, NULL, NULL, hDC, (RECTL*) &m_pOwner->m_rcItem, (RECTL*) &m_pOwner->m_rcItem, NULL, NULL);
...
{
   TRACE(_SC("AX: CActiveXCtrl::OnShowWindow"));
   return E_NOTIMPL;
}

STDMETHODIMP CActiveXCtrl::RequestNewObjectLayout(void)
{
   TRACE(_SC("AX: CActiveXCtrl::RequestNewObjectLayout"));
   return E_NOTIMPL;
}

STDMETHODIMP CActiveXCtrl::CanWindowlessActivate(void)
{
   TRACE(_SC("AX: CActiveXCtrl::CanWindowlessActivate"));
   return S_OK;  // Yes, we can!!
}

STDMETHODIMP CActiveXCtrl::GetCapture(void)
{
   TRACE(_SC("AX: CActiveXCtrl::GetCapture"));
   if( m_pOwner == NULL )
	   return E_UNEXPECTED;
...
{
   TRACE(_SC("AX: CActiveXCtrl::SetCapture"));
   if( m_pOwner == NULL ) 
	   return E_UNEXPECTED;
...
{
   TRACE(_SC("AX: CActiveXCtrl::GetFocus"));
   if( m_pOwner == NULL ) 
	   return E_UNEXPECTED;
...
{
   TRACE(_SC("AX: CActiveXCtrl::SetFocus"));
   if( m_pOwner == NULL ) 
	   return E_UNEXPECTED;
   if( fFocus ) 
	   m_pOwner->SetFocus();
...
{
   TRACE(_SC("AX: CActiveXCtrl::GetDC"));
   if( phDC == NULL ) 
	   return E_POINTER;
   if( m_pOwner == NULL ) 
	   return E_UNEXPECTED;
...
{
   TRACE(_SC("AX: CActiveXCtrl::ReleaseDC"));
   if( m_pOwner == NULL )
	   return E_UNEXPECTED;
   ::ReleaseDC(m_pOwner->m_hwndHost, hDC);
   return S_OK;
...
{
   TRACE(_SC("AX: CActiveXCtrl::InvalidateRect"));
   if( m_pOwner == NULL ) 
	   return E_UNEXPECTED;
...
{
   TRACE(_SC("AX: CActiveXCtrl::InvalidateRgn"));
   if( m_pOwner == NULL ) 
	   return E_UNEXPECTED;
...
{
   TRACE(_SC("AX: CActiveXCtrl::OnInPlaceActivateEx"));
...
{
   TRACE(_SC("AX: CActiveXCtrl::OnInPlaceDeactivateEx"));
   m_bInPlaceActive = false;
   if( m_pInPlaceObject != NULL ) 
   {
      m_pInPlaceObject->Release();
...
{
   TRACE(_SC("AX: CActiveXCtrl::RequestUIActivate"));
   return S_OK;
}

STDMETHODIMP CActiveXCtrl::CanInPlaceActivate(void)       
{
   TRACE(_SC("AX: CActiveXCtrl::CanInPlaceActivate"));
   return S_OK;
}

STDMETHODIMP CActiveXCtrl::OnInPlaceActivate(void)
{
   TRACE(_SC("AX: CActiveXCtrl::OnInPlaceActivate"));
   BOOL bDummy = FALSE;
...
{
   TRACE(_SC("AX: CActiveXCtrl::OnUIActivate"));
   m_bUIActivated = true;
   return S_OK;
}
...
{
   TRACE(_SC("AX: CActiveXCtrl::GetWindowContext"));
   if( ppDoc == NULL ) 
	   return E_POINTER;
   if( ppFrame == NULL ) 
	   return E_POINTER;
...
{
   TRACE(_SC("AX: CActiveXCtrl::Scroll"));
   return E_NOTIMPL;
}

STDMETHODIMP CActiveXCtrl::OnUIDeactivate(BOOL fUndoable)
{
   TRACE(_SC("AX: CActiveXCtrl::OnUIDeactivate"));
   m_bUIActivated = false;
   return S_OK;
}
...
{
   TRACE(_SC("AX: CActiveXCtrl::ContextSensitiveHelp"));
   return S_OK;
}

STDMETHODIMP CActiveXCtrl::OnControlInfoChanged(void)      
{
   TRACE(_SC("AX: CActiveXCtrl::OnControlInfoChanged"));
   return S_OK;
}

STDMETHODIMP CActiveXCtrl::LockInPlaceActive(BOOL fLock)       
{
   TRACE(_SC("AX: CActiveXCtrl::LockInPlaceActive"));
   return S_OK;
}

STDMETHODIMP CActiveXCtrl::GetExtendedControl(IDispatch** ppDisp)        
{
   TRACE(_SC("AX: CActiveXCtrl::GetExtendedControl"));
   if( ppDisp == NULL ) 
	   return E_POINTER;   
   if( m_pOwner == NULL )
	   return E_UNEXPECTED;
...
{
   TRACE(_SC("AX: CActiveXCtrl::ShowPropertyFrame"));
   return E_NOTIMPL;
}

STDMETHODIMP CActiveXCtrl::EnumObjects(DWORD grfFlags, IEnumUnknown** ppenum)
{
   TRACE(_SC("AX: CActiveXCtrl::EnumObjects"));
   if( ppenum == NULL ) 
	   return E_POINTER;
   if( m_pOwner == NULL ) 
	   return E_UNEXPECTED;
...
{
   TRACE(_SC("AX: CActiveXCtrl::LockContainer"));
...
{
   TRACE(_SC("AX: CActiveXCtrl::ParseDisplayName"));
   return E_NOTIMPL;
}
...
LPCTSTR CActiveXWnd::GetWindowClassName() 
{
...
LRESULT CActiveXWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   if( !bHandled ) return CWindowHost::HandleMessage(uMsg, wParam, lParam);
   return lRes;
}
...
CActiveXUI::CActiveXUI(SquirrelObject&so) :CControlUI(so), m_pUnk(NULL), m_pControl(NULL), m_hwndHost(NULL), m_bCreated(false)
{
...
void CActiveXUI::SetPos(RECT rc)
{
   ...
   if( !m_pControl->m_bWindowless )
   {
      ...
      ::MoveWindow(*m_pControl->m_pWindow, m_rcItem.left, m_rcItem.top, pxSize.cx, pxSize.cy, TRUE);
   }
}
