...
CListElementUI::CListElementUI(SquirrelObject& so):CControlUI(so),
   m_iIndex(-1),
   m_pOwner(NULL), 
   m_bSelected(false)
{
...
bool CListElementUI::Activate()
{
   if( !CControlUI::Activate() )
	   return false;
   if( m_pManager != NULL )
	   m_pManager->SendNotify(this, _T("itemactivate"));
   return true;
}
...
CListHeaderUI::CListHeaderUI(SquirrelObject& so):CHorizontalLayoutUI(so)
{
   SetInset(CSize(0, 0));
}
...
CListHeaderItemUI::CListHeaderItemUI(SquirrelObject& so) : CControlUI(so)
,m_uDragState(0)
{
}
...
CListFooterUI::CListFooterUI(SquirrelObject& so):CHorizontalLayoutUI(so)
{

	SquirrelObject rt=SquirrelVM::GetRootTable();

   SquirrelObject ih=SquirrelVM::CreateInstance(rt.GetValue(_T("LabelPanel")));
   CLabelPanelUI* pLabel = new CLabelPanelUI(ih);
   ih.SetInstanceUP(pLabel);
   Add(pLabel);
   ih=SquirrelVM::CreateInstance(rt.GetValue(_T("PaddingPanel")));
   CPaddingPanelUI* pPadding = new CPaddingPanelUI(ih);
   Add(pPadding);
   ih.SetInstanceUP(pPadding);
}
...
{
   if( _tcscmp(pstrName, _T("ListFooter")) == 0 )
	   return this;
...
CListUI::CListUI(SquirrelObject& so):CVerticalLayoutUI(so),
m_pCallback(NULL), m_iCurSel(-1), m_iExpandedItem(-1)
{


	SquirrelObject rt=SquirrelVM::GetRootTable();

   SquirrelObject ih=SquirrelVM::CreateInstance(rt.GetValue(_T("VerticalLayout")));
   m_pList = new CVerticalLayoutUI(ih);
   ih.SetInstanceUP(m_pList);
   ih=SquirrelVM::CreateInstance(rt.GetValue(_T("ListHeader")));
   m_pHeader = new CListHeaderUI(ih);
   ih.SetInstanceUP(m_pHeader);
   ih=SquirrelVM::CreateInstance(rt.GetValue(_T("ListFooter")));
   m_pFooter = new CListFooterUI(ih);
   ih.SetInstanceUP(m_pFooter);

    ih=SquirrelVM::CreateInstance(rt.GetValue(_T("WhiteCanvas")));

   CWhiteCanvasUI* pWhite = new CWhiteCanvasUI(ih);
   pWhite->Add(m_pList);
   ih.SetInstanceUP(pWhite);
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
   ...
   else if( _tcscmp(pstrName, _T("footer")) == 0 ) 
	   GetFooter()->SetVisible(_tcscmp(pstrValue, _T("hidden")) != 0);
   else if( _tcscmp(pstrName, _T("expanding")) == 0 )
	   SetExpanding(_tcscmp(pstrValue, _T("true")) == 0);
   else 
...
CListLabelElementUI::CListLabelElementUI(SquirrelObject& so) :CListElementUI(so) ,
m_cxWidth(0), m_uTextStyle(DT_VCENTER | DT_NOPREFIX | DT_END_ELLIPSIS), m_uButtonState(0)
{
}
...
CListTextElementUI::CListTextElementUI(SquirrelObject& so) :CListLabelElementUI(so),
m_cyItem(0), m_nLinks(0), m_pOwner(NULL)
{
...
void CListTextElementUI::SetOwner(CControlUI* pOwner)
{
   ...
   m_pOwner = static_cast<IListUI*>(pOwner->GetInterface(_SC("List")));
}
...
CListExpandElementUI::CListExpandElementUI(SquirrelObject& so) :CListTextElementUI(so),
m_bExpanded(false), m_cyExpanded(0), m_pContainer(NULL)
{
}
...
bool CListExpandElementUI::Expand(bool bExpand)
{
   ...
   if( bExpand == m_bExpanded ) 
	   return true;
   m_bExpanded = bExpand;
   if( !m_pOwner->ExpandItem(m_iIndex, bExpand) )
	   return false;
   ...
   if( bExpand ) 
   {
	  ...
	  SquirrelObject ih=SquirrelVM::CreateInstance(rt.GetValue(_T("TileLayout")));

      CTileLayoutUI* pTile = new CTileLayoutUI(ih);
	  ih.SetInstanceUP(pTile);
      pTile->SetPadding(4);
      m_pContainer = pTile;
      if( m_pManager != NULL )
		  m_pManager->SendNotify(this, _T("itemexpand"));
      m_pManager->InitControls(m_pContainer, this);
   }
   else
   {
      if( m_pManager != NULL ) 
		  m_pManager->SendNotify(this, _T("itemcollapse"));
   }
   m_cyExpanded = 0;
   return true;
}
