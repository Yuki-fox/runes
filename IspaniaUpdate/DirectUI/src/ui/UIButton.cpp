...
CButtonUI::CButtonUI(SquirrelObject&so) : CControlUI(so),
m_cxWidth(0), m_uButtonState(0), m_uTextStyle(DT_SINGLELINE | DT_CENTER)
{
   m_szPadding.cx = 4;
   m_szPadding.cy = 0;
}
...
{
	int height= m_pManager? ( m_pManager->GetThemeFontInfo(UIFONT_NORMAL).tmHeight):12;
   SIZE sz = { m_cxWidth, 12 +height };
...
COptionUI::COptionUI(SquirrelObject&so) : CControlUI(so),
m_cxWidth(0), m_uButtonState(0), m_uStyle(DT_LEFT), m_bSelected(false)
{
}
...
CCheckUI::CCheckUI(SquirrelObject&so) : CControlUI(so),
m_cxWidth(0), m_uButtonState(0), m_uStyle(DT_LEFT), m_bSelected(false)
{
}

LPCTSTR CCheckUI::GetClass() const
{
	return _T("CheckUI");
}

UINT CCheckUI::GetControlFlags() const
{
	return UIFLAG_TABSTOP;
}

void CCheckUI::Event(TEventUI& event)
{
	if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
	{
		if( ::PtInRect(&m_rcItem, event.ptMouse) && IsEnabled() ) {
...
bool CCheckUI::IsChecked() const
{
	return m_bSelected;
}

void CCheckUI::SetCheck(bool bSelected)
{
...
void CCheckUI::SetWidth(int cxWidth)
{
	m_cxWidth = cxWidth;
	UpdateLayout();
}
...
SIZE CCheckUI::EstimateSize(SIZE /*szAvailable*/)
{
	return CSize(m_cxWidth, 2 + m_pManager->GetThemeFontInfo(UIFONT_NORMAL).tmHeight);
}

void CCheckUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
	// Draw button
	UINT uState = 0;
	if( m_bSelected )
	...
	if( !IsEnabled() )
		uState |= UISTATE_DISABLED;
	CBlueRenderEngineUI::DoPaintCheckBox(hDC, m_pManager, m_rcItem, m_sText, m_uButtonState | uState, m_uStyle);
}
