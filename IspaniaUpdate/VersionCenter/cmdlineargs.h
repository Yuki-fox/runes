...
class CmdLineArgs : public base::Array<TCHAR*>
{
public:
    CmdLineArgs ()
    {
        ...
        m_cmdline = new TCHAR [lstrlen (cmdline) + 1];
        if (m_cmdline)
        {
            lstrcpy (m_cmdline, cmdline);
            ParseCmdLine(); 
        }
    }
    ~CmdLineArgs()
...
private:
    LPTSTR m_cmdline; // the command line string
    ...
    void ParseCmdLine ()
    {
        ...
        while (*pargs)
        {
            ...
            add (pargs);              // store position of current token
