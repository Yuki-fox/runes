...
NOTES:			Provides an interface to the CMD5Checksum class. 'strFilePath' name should 
				...
				exception.
*****************************************************************************************/
BOOL CMD5Checksum::GetMD5(LPCTSTR  strFilePath,char* OutMD5Str,IMD5_Progress* progress)
{
	//open the file as a binary file in readonly mode, denying write access 

	HANDLE hFile;

	if ( (hFile = CreateFile( strFilePath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL ) ) == INVALID_HANDLE_VALUE ) 
	{
		return FALSE;
	}
	
	//(strFilePath, CFile::modeRead | CFile::shareDenyWrite | CFile::typeBinary);

	strcpy((char*)OutMD5Str ,"error:File not find!!");
	
	
	GetMD5(hFile,OutMD5Str,progress);
	CloseHandle( hFile );
...
*****************************************************************************************/
BOOL CMD5Checksum::GetMD5(HANDLE hFile,char* OutMD5Str,IMD5_Progress* progress)
{

	try
	{
		CMD5Checksum MD5Checksum;		//checksum object	
		MD5Checksum.setProgress(progress);
	//	int nLength = 0;				//number of bytes read from the file
		...
		if (progress)
			progress->progress_function(size,0);
		
		while(1) 
		{

			ReadFile(hFile,Buffer,1024,&readBytes,NULL);
			if (readBytes>0)
			{
				MD5Checksum.Update( Buffer, readBytes );
				sizeStep+=readBytes;
				if (progress)
					progress->progress_function(size,sizeStep);
		...
		return MD5Checksum.Final(OutMD5Str);
	}

	//report any file exceptions in debug mode only
	catch (... )
	{
		base::Debug::printf(_TEXT("CMD5Checksum::GetMD5: CFileException caught"));	
...
*****************************************************************************************/
BOOL CMD5Checksum::GetMD5(BYTE* pBuf, UINT nLength,char* OutMD5Str,IMD5_Progress* progress)
{
	//entry invariants
...
{
	//entry invariants
//	ATLASSERT( nLength % 4 == 0 );
//	ATLASSERT( AfxIsValidAddress(Output, nLength/4, TRUE) );
//	ATLASSERT( AfxIsValidAddress(Input, nLength, FALSE) );
...
void CMD5Checksum::DWordToByte(BYTE* Output, DWORD* Input, UINT nLength )
{
	//entry invariants
//	ATLASSERT( nLength % 4 == 0 );
//	ATLASSERT( AfxIsValidAddress(Output, nLength, TRUE) );
//	ATLASSERT( AfxIsValidAddress(Input, nLength/4, FALSE) );
...
*****************************************************************************************/
BOOL CMD5Checksum::Final(char* OutputMD5)
{
	//Save number of bits
	BYTE Bits[8];
